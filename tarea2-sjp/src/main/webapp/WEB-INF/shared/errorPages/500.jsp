...
<body id="error_page">
    <div id="body-container">
        ...
        <div class="main">
            <% 
                // Try to get exception from implicit object or request attribute
                Throwable error = exception;
                if (error == null) {
                    error = (Throwable) request.getAttribute("jakarta.servlet.error.exception");
                }
                
                // Get error message from request attribute if exists
                String errorMessage = (String) request.getAttribute("error");
                
                if (error != null) {
            %>
                <pre><%= error.getClass().getName() %>: <%= error.getMessage() %>
<% for(StackTraceElement ste: error.getStackTrace()){ %><%= ste.toString() %>
<% } %></pre>
            <% } else if (errorMessage != null) { %>
                <div style="color: #dc3545; font-weight: bold; padding: 20px;">
                    <%= errorMessage %>
                </div>
            <% } else { %>
                <div style="color: #dc3545; font-weight: bold; padding: 20px;">
